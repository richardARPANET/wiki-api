...
class WikiApi(object):
    ...
    def get_article(self, title):
        ...
        # Update image extraction to use og:image meta tag
        og_image = html('meta[property="og:image"]').attr("content")
        data["image"] = og_image if og_image else None
    ...
    def get_tables(self, url):
        ...
        for table_idx, table in enumerate(tables):
            ...
            if caption and caption.get_text().strip():
                ...
                if prev_heading:
                    caption_text = prev_heading.get_text().strip()
                else:
                    caption_text = f"Table {table_idx + 1}"

            caption_text = re.sub(r"\[edit\]", "", caption_text)
            caption_text = re.sub(r"\[\d+\]", "", caption_text)
            caption_text = caption_text.strip()
            df = pd.read_html(str(table))[0]
            tables_out[caption_text] = df
        return tables_out
